...
|------|----------|---------------|
| Backend | Django 6.x (Python 3.12) | Admin integrado, auth robusto, ORM, velocidad de desarrollo |
| Frontend | Templates de Django + **Alpine.js 3** | Interactividad ligera (wizard de cuestionario, autosave, filtros) sin SPA |
| Estilos | **Tailwind CSS** (vía `django-tailwind` o CLI standalone) | Sistema de diseño utilitario, build simple |
| BD | SQLite en desarrollo → **PostgreSQL** en producción | Sin fricción al inicio, robustez en prod |
...
| Despliegue | Docker + gunicorn + whitenoise (estáticos) | Portabilidad |
| Auditoría | `django-simple-history` en modelos críticos | Trazabilidad de cambios en evaluaciones y catálogos |

**Principios:**
- Server-side rendering; Alpine solo para UX (sin API REST pública en v1; endpoints JSON internos para autosave).
...
## 7. Pantallas y UX (con Alpine.js)
...
Proyecto Django, settings por ambiente, Docker, Tailwind+Alpine integrados, CI básico (lint + tests), layout base con navegación por rol (mock).
...
Auditoría (`django-simple-history`), matriz completa de tests de permisos, pruebas de carga ligeras, backups, despliegue a producción, manual de usuario breve por rol, sesión de capacitación a Talento.
